import { ChangeDetectionStrategy, Component, Input, computed, inject, signal, OnChanges, SimpleChanges } from '@angular/core';
...
import { AssignmentItem, SubmissionItem } from '../../../entities/course/model/assignment.types';
...
export class LessonViewerComponent implements OnChanges {
  ...
  readonly hasVideo = computed(() => this.lesson.resources?.some(r => r.resource_type === 'VIDEO') ?? false);
  readonly mainVideo = computed(() => this.lesson.resources?.find(r => r.resource_type === 'VIDEO') ?? null);
  ...
  private async loadAssignmentData(lessonId: number): Promise<void> {
    try {
      const assignments = await firstValueFrom(this.assignmentApi.getAssignmentsByLesson(lessonId));
      if (assignments && assignments.length > 0) {
        const activeAssignment = assignments[0];
        this.assignment.set(activeAssignment);
        
        // Verifica si el estudiante ya entregó esta tarea
        const submissions = await firstValueFrom(this.assignmentApi.getSubmissionsByAssignment(activeAssignment.id));
  ...
  async onFileSelected(event: Event): Promise<void> {
    ...
    try {
      const result = await firstValueFrom(this.assignmentApi.submitAssignment(assignmentId, studentId, file));
      this.submission.set(result); // Mostramos el success
    } catch {
      this.uploadError.set('Error subiendo el archivo. Por favor, intenta de nuevo.');
    } finally {
