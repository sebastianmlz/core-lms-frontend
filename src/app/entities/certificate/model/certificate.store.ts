...
export const CertificateStore = signalStore(
  ...
  withMethods(
    (
      store,
      api = inject(CertificateApiService),
      toast = inject(GlobalToastService),
    ) => ({
      async generate(
        courseId: number,
        studentId: number,
        ...
        try {
          const result = await firstValueFrom(
            api.generateCertificate(courseId, studentId),
          );
          patchState(store, { isGenerating: false });
          toast.success(
            '¡Certificado generado!',
            'Redirigiendo a tu diploma digital...',
          );
      ...
      async verify(hash: string): Promise<void> {
        ...
        try {
          ...
          patchState(store, {
            isVerifying: false,
            error: 'Certificado inválido o no encontrado.',
          });
        }
      },
