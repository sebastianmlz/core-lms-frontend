...
function isAdaptivePlanResponse(value: unknown): value is AdaptivePlanResponse {
  return (
    isRecord(value) && Array.isArray(value['items']) && isRecord(value['_meta'])
  );
}

function isAdaptivePlanFallback(
  value: unknown,
): value is AttemptAdaptiveFallback {
  return isRecord(value) && value['fallback'] === true;
}

function buildFallbackReason(
  fallbackEnvelope: AttemptAdaptiveFallback,
): string {
  if (
    typeof fallbackEnvelope.reason === 'string' &&
    fallbackEnvelope.reason.trim().length > 0
  ) {
...
export const ReasoningStore = signalStore(
  ...
  withComputed((store) => ({
    ...
    hasPendingDiagnostic: computed(
      () => store.diagnosticStatus() === 'pending',
    ),
  })),
  withMethods(
    (
      store,
      reasoningApi = inject(ReasoningApiService),
      attemptApi = inject(AttemptApiService),
    ) => ({
      async generateAdaptivePlan(input: AdaptivePlanInput): Promise<void> {
        ...
        try {
          const plan = await firstValueFrom(
            reasoningApi.generateAdaptivePlan(input),
          );
          patchState(store, {
            adaptivePlan: plan,
            isLoadingPlan: false,
      ...
      async runDiagnosticFromAttempt(input: AttemptSubmitInput): Promise<void> {
        ...
        try {
          ...
          const jobId =
            ...
            (isRecord(attempt.adaptive_plan)
              ? normalizeJobId(attempt.adaptive_plan['job_id'])
              : null);
      ...
      applyAttemptResult(attempt: AttemptResultResponse): void {
        ...
        if (attempt.axiom_error) {
          patchState(store, {
            ...
            lastAttemptId: attempt.id,
            error:
              attempt.axiom_error.details ||
              'El motor adaptativo retorno un error.',
          });
          return;
        }
      ...
      async loadCognitiveGraph(
        ...
        try {
          const graph = await firstValueFrom(
            reasoningApi.getCognitiveGraph(studentId, topics, targetTopic),
          );
          patchState(store, {
