  ...
  private async loadAssignmentAndSubmissions(lessonId: number): Promise<void> {
    ...
    try {
      ...
      if (assignments && assignments.length > 0) {
        ...
        const subs = await firstValueFrom(this.assignmentApi.getSubmissionsByAssignment(activeAssignment.id));
  ...
  async saveGrade(submission: SubmissionItem): Promise<void> {
    ...
    try {
      const result = await firstValueFrom(
        this.client.patch<SubmissionItem, GradingPayload>(`/api/v1/submissions/${submission.id}/grade/`, { grade })
      );
