...
import { CourseDashboardSummary, CourseDetail, CourseListItem } from '../model/course.types';

@Injectable({ providedIn: 'root' })
export class CourseApiService {
  private readonly djangoApi = inject(DjangoApiClient);

  listCourses(params?: { semester?: number; career?: number }): Observable<PaginatedResponse<CourseListItem>> {
    return this.djangoApi.get<PaginatedResponse<CourseListItem>>('/api/v1/courses/', {
      params: {
  ...
  getCourseDashboard(courseId: number): Observable<CourseDashboardSummary> {
    return this.djangoApi.get<CourseDashboardSummary>(`/api/v1/analytics/course/${courseId}/dashboard/`);
  }
}
